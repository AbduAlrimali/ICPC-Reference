 ...
 * License: CC0
 * Source: user
 * Description: Li Chao Tree for line and segment minimum queries.
 * Supports add\_line({m, c}), add\_segment({m, c}, l, r), and query(x).
 * For maximum, insert negated lines {-m, -c} and negate the query result.
 * Time: $O(\log N)$
 * Status: unknown
 */
#pragma once
